...
type MerkleTree = ark_crypto_primitives::merkle_tree::MerkleTree<MerkleConfig>;
...
type Path = ark_crypto_primitives::merkle_tree::Path<MerkleConfig>;
...
fn main() {
    ...
    let mut tree = MerkleTree::blank(&leaf_hash_params, &inner_hash_params, 5).unwrap();
